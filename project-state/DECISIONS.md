...
### D028 — APPROVED: DHT22 supplies both `temperature` (ambient air) and `humidity`; DS18B20 is an optional alternate
- **Date:** 2026-09-09 (recorded), **approved** by the project supervisor.
- **Status:** APPROVED PERMANENT PROJECT CHOICE. Not a substitution, not a
  stopgap, not a deviation pending correction. This is the project's
  temperature configuration; the sensor set below is what SHTAPM is built
  and evaluated on.

- **Decision:** the `temperature` channel is sourced from the DHT22's
  on-chip ambient-air temperature reading
  (`DHT22AdafruitTemperatureDriver`, `edge/drivers/dht22_adafruit.py`), the
  same physical sensor that supplies `humidity`. Both channels are served
  through one shared reader per `(pin, use_pulseio)`, so they cost a single
  hardware measurement per acquisition cycle. `temperature` is registered as
  a normal real driver (`registry._REAL_DRIVER_CLASSES["temperature"]`) and
  is selected by `DriverSpec(kind="real")` like any other channel.

- **DS18B20: optional alternate, NOT required.** `edge/drivers/ds18b20.py`
  remains fully implemented, tested, and registered as this channel's
  alternate (`registry._ALTERNATE_DRIVER_CLASSES["temperature"]`). The
  project does not require that hardware and is not blocked on it. If it is
  ever added, selecting it is one explicit change —
  `DriverSpec(kind="alternate")` in `edge/main.py`'s
  `_DEFAULT_CHANNEL_SPECS`, or `SHTAPM_DRIVER_TEMPERATURE=alternate` for a
  single run — with no pipeline, contract, topic, schema, or consumer
  change. The registry never selects it implicitly; there is no
  real→alternate fallback path.

- **What this channel measures (specification, not caveat).** These are
  properties of the approved configuration and belong in any description,
  model, or write-up that uses these channels:
  1. `temperature` is **ambient air temperature at the sensor** — an
     indicative environmental measurement, not a contact reading of a motor
     or bearing surface. The DS18B20 alternate measures a different physical
     quantity, so a model fitted on one is not automatically valid for the
     other.
  2. `temperature` and `humidity` come from **one part reading one air
     mass**. They are physically coupled and share a failure mode, so any
     method that treats channels as independent evidence — cross-sensor
...
- **Effect on modelling and evidence.** Because this is the permanent
  ...
  representative of the system as built, and is usable for U05 work on that
  basis. Two constraints follow from the measurement characteristics above,
  and are binding on any claim made from such data:
  - The digital twin reconstructs each masked channel from the others.
    Strong `temperature`↔`humidity` agreement is expected here and reflects
    a shared sensor, **not** independent physical corroboration; it must
    not be reported as cross-channel validation.
  - `edge/trust/k_correlation.py` is unaffected as implemented — it
    constrains only the current↔vibration pair and assigns `k=1.0` to
    temperature and humidity (D010). Any future cross-sensor correlation
    term covering this pair must treat them as coupled by construction.
  - If the DS18B20 alternate is ever adopted, the measured quantity on this
    channel changes; models and thresholds fitted on ambient air must be
    refitted, not carried over.

- **Labelling.** The frozen wire contract carries no sensor identity (same
  field, same `°C` unit either way, deliberately unchanged), so the source
  is named where it can be: `edge/main.py` prints the active temperature
  source on every start, and the driver/registry/bench-wiring-test
  docstrings carry the measurement characteristics.

- **Does NOT resolve:** U05 (`divergence_threshold` remains open and
  data-gated — this record makes bench data admissible for that work, it
  does not supply the value or any threshold); the P1 physical acquisition
  gate; gas (MQ-135, still no driver) or current (INA219, implemented but
  not connected). **Does NOT change:** D001–D027; the frozen six-channel
  contract; `edge/drivers/ds18b20.py` (untouched, still tested); INA219 or
  MQ-135. **No accuracy or calibration claim** is made for the DHT22
  temperature reading — it has not been validated against a reference
  thermometer.

- **Supersedes:** this entry's own earlier form, recorded the same day, which
  described the configuration as a temporary operator-approved substitution
  for an undetected DS18B20 and barred the resulting data from U05 evidence.
  The supervisor decision replaces that framing and lifts that bar, subject
  to the two modelling constraints stated above.
...
**What this increment explicitly does NOT do:**
- **Does not complete §4's proposed minimum** — that applies to every scenario in the taxonomy (the 8 injection-type scenarios plus this one); broader 5-seed coverage across the other 8 scenarios remains unfinished.
...
## U06 — Seed-Repetition Report (Injected Current Spike) Implementation Note (IMPLEMENTATION RECORD, NOT A DECISION)
**(U06 REMAINS FULLY UNDECIDED/OPEN. This is an implementation record for the second U06 seed-repetition increment — it establishes 5-seed coverage for a second scenario shape (injected_current_spike), per the "U06 -- RL REWARD SHAPING SPECIFICATION PROPOSAL" section's own §4 proposed minimum. It defines no new axis, metric, threshold, or verdict, and does not resolve U06.)**

- **Date implemented:** 2026-09-06
- **Files changed:** `edge/eval/rl_baseline_eval.py` (additive only — 4 new `ScenarioConfig` constants plus one grouping tuple; no existing scenario, function signature, or behavior changed), `edge/eval/u06_seed_repetition_report_injected_current_spike.py` (new), `edge/tests/test_u06_seed_repetition_report_injected_current_spike.py` (new). No other file modified — `EpisodeRecord`, `TransitionRecord`, `active_injection_labels`, all three existing axis modules, `u06_diagnostic_report.py`, the existing clean-degradation `u06_seed_repetition_report.py`, `edge/eval/rl_training.py`, `edge/rl/reward.py`, `edge/rl/fallback_gate.py`, `edge/rl/policy.py`, `edge/rl/environment.py`, and every hardware/driver/actuator/relay/GPIO file are untouched.

**What this increment does:**
- Adds `SCENARIO_INJECTED_CURRENT_SPIKE_SEED_1350/1351/1352/1353` (seeds 1350–1353, none colliding with any existing evaluation-scenario seed 1337–1349, the clean-degradation seed-repetition seeds 1346–1349, or `TRAINING_SCENARIOS` seed 2001/2002), each reusing `SCENARIO_INJECTED_CURRENT_SPIKE`'s exact profile (length, health range, degradation rate, channel config, and its `Spike` injection unchanged) with only `seed` differing, plus a grouping tuple `INJECTED_CURRENT_SPIKE_SEED_REPETITION_SCENARIOS` (the original plus all four variants — 5 total).
- Adds a pure function, `build_seed_repetition_report()`, in a new sibling module mirroring the clean-degradation seed-repetition report's exact structure — running both existing baselines over all 5 seeds and calling all three existing axis summary functions unmodified on each of the resulting 10 `EpisodeRecord`s. No average, pooled rate, min, max, range, or variability statistic across seeds is computed anywhere (confirmed by the same AST-based division/builtin-statistic checks used in the clean-degradation increment).
- **This is now the second of nine scenario shapes with 5-seed coverage.** Includes an optional `main()` printer matching the existing diagnostic-entry convention.
- **Empirical note (diagnostic, not a claim):** for this scenario and episode length, all 5 seeds produce byte-identical `cumulative_reward`/`final_health`/axis-summary values — the same behavior already observed for the clean-degradation seed set. This appears to be a property of the degradation generator/scenario length combination at this scale, not a defect introduced by this increment; it is reported here for transparency, not investigated or resolved by this increment.

**What this increment explicitly does NOT do:**
- **Does not complete §4's proposed minimum** — 7 injection-type scenarios (Drift, StuckAt, BiasFDI, RampFDI, Replay, ConstantSpoof, AdaptiveStealthFDI) still have only their single, original seed each; broader 5-seed coverage across them remains unfinished.
- Defines no new U06 axis, comparison, definition, denominator, or metric.
- Computes no threshold, verdict, or pass/fail judgment, and makes no real-world accuracy, safety, effectiveness, validation, or production-readiness claim anywhere.
- Does not implement channel matching or DQN-policy evaluation.
- Does not modify the existing clean-degradation seed-repetition module — both exist side by side, structurally identical in shape.
- No permanent numeric snapshot from running this report is pasted into this document.
...
**What this increment explicitly does NOT do:**
- **Does not complete §4's proposed minimum** — 6 injection-type scenarios (StuckAt, BiasFDI, RampFDI, Replay, ConstantSpoof, AdaptiveStealthFDI) still have only their single, original seed each; broader 5-seed coverage across them remains unfinished.
- Defines no new U06 axis, comparison, definition, denominator, or metric.
- Computes no threshold, verdict, or pass/fail judgment, and makes no real-world accuracy, safety, effectiveness, validation, or production-readiness claim anywhere.
- Does not implement channel matching or DQN-policy evaluation.
...
## U06 — Seed-Repetition Report (Injected Pressure Stuck-At) Implementation Note (IMPLEMENTATION RECORD, NOT A DECISION)
**(U06 REMAINS FULLY UNDECIDED/OPEN. This is an implementation record for the fourth U06 seed-repetition increment — it establishes 5-seed coverage for a fourth scenario shape (injected_pressure_stuck_at), per the "U06 -- RL REWARD SHAPING SPECIFICATION PROPOSAL" section's own §4 proposed minimum. It defines no new axis, metric, threshold, or verdict, and does not resolve U06.)**

- **Date implemented:** 2026-09-06
- **Files changed:** `edge/eval/rl_baseline_eval.py` (additive only — 4 new `ScenarioConfig` constants plus one grouping tuple; no existing scenario, function signature, or behavior changed), `edge/eval/u06_seed_repetition_report_injected_pressure_stuck_at.py` (new), `edge/tests/test_u06_seed_repetition_report_injected_pressure_stuck_at.py` (new). No other file modified — `EpisodeRecord`, `TransitionRecord`, `active_injection_labels`, all three existing axis modules, `u06_diagnostic_report.py`, the existing clean-degradation, injected_current_spike, and injected_temperature_drift seed-repetition modules, `edge/eval/rl_training.py`, `edge/rl/reward.py`, `edge/rl/fallback_gate.py`, `edge/rl/policy.py`, `edge/rl/environment.py`, and every hardware/driver/actuator/relay/GPIO file are untouched.

**What this increment does:**
- Adds `SCENARIO_INJECTED_PRESSURE_STUCK_AT_SEED_1358/1359/1360/1361` (seeds 1358–1361, none colliding with any existing evaluation-scenario seed 1337–1357 or `TRAINING_SCENARIOS` seed 2001/2002), each reusing `SCENARIO_INJECTED_PRESSURE_STUCK_AT`'s exact profile (length, health range, degradation rate, channel config, and its `StuckAt` injection unchanged) with only `seed` differing, plus a grouping tuple `INJECTED_PRESSURE_STUCK_AT_SEED_REPETITION_SCENARIOS` (the original plus all four variants — 5 total).
- Adds a pure function, `build_seed_repetition_report()`, in a new sibling module mirroring the three existing seed-repetition reports' exact structure — running both existing baselines over all 5 seeds and calling all three existing axis summary functions unmodified on each of the resulting 10 `EpisodeRecord`s. No average, pooled rate, min, max, range, or variability statistic across seeds is computed anywhere (confirmed by the same AST-based division/builtin-statistic checks used in all three prior increments).
- **This is now the fourth of nine scenario shapes with 5-seed coverage.** Includes an optional `main()` printer matching the existing diagnostic-entry convention.
- **Empirical note (diagnostic, not a claim):** for this scenario and episode length, all 5 seeds produce byte-identical `cumulative_reward`/`final_health`/axis-summary values — the same behavior already observed for the clean-degradation, injected_current_spike, and injected_temperature_drift seed sets (now the fourth scenario shape to show the identical pattern). This is reported as a diagnostic observation about the current degradation-generator/scenario scale, for transparency only — it is NOT investigated, explained, or resolved by this increment, and must NOT be read as proof that seed variation is impossible, that executions were pooled or shared, or that the policy is accurate or safe. This disclaimer is documented directly in the module's own docstring, not only in this note.
- **StuckAt-on-pressure scenario note:** the module docstring also documents, as a characteristic of this already-committed synthetic scenario (not a real-world conclusion), that `pressure` is unconfigured in the shared evaluation profile and is generated as a flat, seed-independent baseline constant on every frame — so `StuckAt(held_value=None)` freezing it at its onset value is, for this specific scenario's configuration, observably a no-op against its own ambient signal. Verified directly: the injected and uninjected `pressure` streams are byte-for-byte identical at every frame. This is documented as a scenario-specific characteristic only, explicitly not a claim about real stuck-at sensor behavior or this pipeline's real-world fault-detection capability.

**What this increment explicitly does NOT do:**
- **Does not complete §4's proposed minimum** — 5 injection-type scenarios (BiasFDI, RampFDI, Replay, ConstantSpoof, AdaptiveStealthFDI) still have only their single, original seed each; broader 5-seed coverage across them remains unfinished.
- Defines no new U06 axis, comparison, definition, denominator, or metric.
- Computes no threshold, verdict, or pass/fail judgment, and makes no real-world accuracy, safety, effectiveness, validation, or production-readiness claim anywhere.
- Does not implement channel matching or DQN-policy evaluation.
- Does not modify any of the three prior seed-repetition modules — all four exist side by side, structurally identical in shape.
- No permanent numeric snapshot from running this report is pasted into this document.

**Not done by this increment:** U06's status in the UNDECIDED list above is unchanged: fully open, zero partial resolution. Broader 5-seed coverage across the remaining 5 scenarios, channel-matched comparison, DQN-policy inclusion, and any human sign-off accepting the Operational Definitions Proposal all remain open, separate questions.

---

## U06 — Seed-Repetition Report (Injected Humidity Bias FDI) Implementation Note (IMPLEMENTATION RECORD, NOT A DECISION)
**(U06 REMAINS FULLY UNDECIDED/OPEN. This is an implementation record for the fifth U06 seed-repetition increment — it establishes 5-seed coverage for a fifth scenario shape (injected_humidity_bias_fdi), per the "U06 -- RL REWARD SHAPING SPECIFICATION PROPOSAL" section's own §4 proposed minimum. It defines no new axis, metric, threshold, or verdict, and does not resolve U06.)**

- **Date implemented:** 2026-09-06
- **Files changed:** `edge/eval/rl_baseline_eval.py` (additive only — 4 new `ScenarioConfig` constants plus one grouping tuple; no existing scenario, function signature, or behavior changed), `edge/eval/u06_seed_repetition_report_injected_humidity_bias_fdi.py` (new), `edge/tests/test_u06_seed_repetition_report_injected_humidity_bias_fdi.py` (new). No other file modified — `EpisodeRecord`, `TransitionRecord`, `active_injection_labels`, all three existing axis modules, `u06_diagnostic_report.py`, the existing clean-degradation, injected_current_spike, injected_temperature_drift, and injected_pressure_stuck_at seed-repetition modules, `edge/eval/rl_training.py`, `edge/rl/reward.py`, `edge/rl/fallback_gate.py`, `edge/rl/policy.py`, `edge/rl/environment.py`, and every hardware/driver/actuator/relay/GPIO file are untouched.

**What this increment does:**
- Adds `SCENARIO_INJECTED_HUMIDITY_BIAS_FDI_SEED_1362/1363/1364/1365` (seeds 1362–1365, none colliding with any existing evaluation-scenario seed 1337–1361 or `TRAINING_SCENARIOS` seed 2001/2002), each reusing `SCENARIO_INJECTED_HUMIDITY_BIAS_FDI`'s exact profile (length, health range, degradation rate, channel config, and its `BiasFDI` injection unchanged) with only `seed` differing, plus a grouping tuple `INJECTED_HUMIDITY_BIAS_FDI_SEED_REPETITION_SCENARIOS` (the original plus all four variants — 5 total).
- Adds a pure function, `build_seed_repetition_report()`, in a new sibling module mirroring the four existing seed-repetition reports' exact structure — running both existing baselines over all 5 seeds and calling all three existing axis summary functions unmodified on each of the resulting 10 `EpisodeRecord`s. No average, pooled rate, min, max, range, or variability statistic across seeds is computed anywhere (confirmed by the same AST-based division/builtin-statistic checks used in all four prior increments).
- **This is now the fifth of nine scenario shapes with 5-seed coverage.** Includes an optional `main()` printer matching the existing diagnostic-entry convention.
- **Empirical note (diagnostic, not a claim):** for this scenario and episode length, all 5 seeds produce byte-identical `cumulative_reward`/`final_health`/axis-summary values — the same behavior already observed for the clean-degradation, injected_current_spike, injected_temperature_drift, and injected_pressure_stuck_at seed sets (now the fifth scenario shape to show the identical pattern). This is reported as a diagnostic observation about the current degradation-generator/scenario scale, for transparency only — it is NOT investigated, explained, or resolved by this increment, and must NOT be read as proof that seed variation is impossible, that executions were pooled or shared, or that the policy is accurate or safe. This disclaimer is documented directly in the module's own docstring, not only in this note.
- **BiasFDI-on-humidity scenario note:** the module docstring also documents, as a characteristic of this already-committed synthetic scenario (not a real-world conclusion), that `humidity` is unconfigured in the shared evaluation profile and is generated as a flat, seed-independent baseline constant on every frame. Unlike the injected_pressure_stuck_at scenario's `StuckAt` injection (a near no-op against its own already-flat baseline), `BiasFDI` here adds a fixed `+5.0` offset that IS observably different from the ambient signal during its active window — verified directly: the injected and uninjected `humidity` streams differ by exactly the configured bias throughout the injection window and are identical outside it. This is documented as a scenario-specific characteristic only, explicitly not a claim about how a real false-data-injection attack would behave or this pipeline's real-world attack-detection capability.

**What this increment explicitly does NOT do:**
- **Does not complete §4's proposed minimum** — 4 injection-type scenarios (RampFDI, Replay, ConstantSpoof, AdaptiveStealthFDI) still have only their single, original seed each; broader 5-seed coverage across them remains unfinished.
- Defines no new U06 axis, comparison, definition, denominator, or metric.
- Computes no threshold, verdict, or pass/fail judgment, and makes no real-world accuracy, safety, effectiveness, validation, or production-readiness claim anywhere.
- Does not implement channel matching or DQN-policy evaluation.
- Does not modify any of the four prior seed-repetition modules — all five exist side by side, structurally identical in shape.
- No permanent numeric snapshot from running this report is pasted into this document.

**Not done by this increment:** U06's status in the UNDECIDED list above is unchanged: fully open, zero partial resolution. Broader 5-seed coverage across the remaining 4 scenarios, channel-matched comparison, DQN-policy inclusion, and any human sign-off accepting the Operational Definitions Proposal all remain open, separate questions.

---

## U06 — Seed-Repetition Report (Injected Gas Ramp FDI) Implementation Note (IMPLEMENTATION RECORD, NOT A DECISION)
**(U06 REMAINS FULLY UNDECIDED/OPEN. This is an implementation record for the sixth U06 seed-repetition increment — it establishes 5-seed coverage for a sixth scenario shape (injected_gas_ramp_fdi), per the "U06 -- RL REWARD SHAPING SPECIFICATION PROPOSAL" section's own §4 proposed minimum. It defines no new axis, metric, threshold, or verdict, and does not resolve U06.)**

- **Date implemented:** 2026-09-06
- **Files changed:** `edge/eval/rl_baseline_eval.py` (additive only — 4 new `ScenarioConfig` constants plus one grouping tuple; no existing scenario, function signature, or behavior changed), `edge/eval/u06_seed_repetition_report_injected_gas_ramp_fdi.py` (new), `edge/tests/test_u06_seed_repetition_report_injected_gas_ramp_fdi.py` (new). No other file modified — `EpisodeRecord`, `TransitionRecord`, `active_injection_labels`, all three existing axis modules, `u06_diagnostic_report.py`, the existing clean-degradation, injected_current_spike, injected_temperature_drift, injected_pressure_stuck_at, and injected_humidity_bias_fdi seed-repetition modules, `edge/eval/rl_training.py`, `edge/rl/reward.py`, `edge/rl/fallback_gate.py`, `edge/rl/policy.py`, `edge/rl/environment.py`, and every hardware/driver/actuator/relay/GPIO file are untouched.

**What this increment does:**
- Adds `SCENARIO_INJECTED_GAS_RAMP_FDI_SEED_1366/1367/1368/1369` (seeds 1366–1369, none colliding with any existing evaluation-scenario seed 1337–1365 or `TRAINING_SCENARIOS` seed 2001/2002), each reusing `SCENARIO_INJECTED_GAS_RAMP_FDI`'s exact profile (length, health range, degradation rate, channel config, and its `RampFDI` injection unchanged) with only `seed` differing, plus a grouping tuple `INJECTED_GAS_RAMP_FDI_SEED_REPETITION_SCENARIOS` (the original plus all four variants — 5 total).
- Adds a pure function, `build_seed_repetition_report()`, in a new sibling module mirroring the five existing seed-repetition reports' exact structure — running both existing baselines over all 5 seeds and calling all three existing axis summary functions unmodified on each of the resulting 10 `EpisodeRecord`s. No average, pooled rate, min, max, range, or variability statistic across seeds is computed anywhere (confirmed by the same AST-based division/builtin-statistic checks used in all five prior increments).
- **This is now the sixth of nine scenario shapes with 5-seed coverage.** Includes an optional `main()` printer matching the existing diagnostic-entry convention.
- **Empirical note (diagnostic, not a claim):** for this scenario and episode length, all 5 seeds produce byte-identical `cumulative_reward`/`final_health`/axis-summary values — the same behavior already observed for the clean-degradation, injected_current_spike, injected_temperature_drift, injected_pressure_stuck_at, and injected_humidity_bias_fdi seed sets (now the sixth scenario shape to show the identical pattern). This is reported as a diagnostic observation about the current degradation-generator/scenario scale, for transparency only — it is NOT investigated, explained, or resolved by this increment, and must NOT be read as proof that seed variation is impossible, that executions were pooled or shared, or that the policy is accurate or safe. This disclaimer is documented directly in the module's own docstring, not only in this note.
- **RampFDI-on-gas scenario note:** the module docstring also documents, as a characteristic of this already-committed synthetic scenario (not a real-world conclusion), that `gas` is unconfigured in the shared evaluation profile and is generated as a flat, seed-independent baseline constant on every frame. Unlike `StuckAt` (no-op) and `BiasFDI` (a single fixed offset), `RampFDI` here adds a monotonically increasing offset across its 8-frame window — empirically verified exact offsets of +0.8, +1.6, +2.4, +3.2, +4.0, +4.8, +5.6, +6.4 relative to the flat ambient value, with the injected and uninjected streams identical outside the window. This is documented as a scenario-specific characteristic only, explicitly not a claim about how a real ramp-style false-data-injection attack would behave or this pipeline's real-world attack-detection capability.

**What this increment explicitly does NOT do:**
- **Does not complete §4's proposed minimum** — 3 injection-type scenarios (Replay, ConstantSpoof, AdaptiveStealthFDI) still have only their single, original seed each; broader 5-seed coverage across them remains unfinished.
- Defines no new U06 axis, comparison, definition, denominator, or metric.
- Computes no threshold, verdict, or pass/fail judgment, and makes no real-world accuracy, safety, effectiveness, validation, or production-readiness claim anywhere.
- Does not implement channel matching or DQN-policy evaluation.
- Does not modify any of the five prior seed-repetition modules — all six exist side by side, structurally identical in shape.
- No permanent numeric snapshot from running this report is pasted into this document.

**Not done by this increment:** U06's status in the UNDECIDED list above is unchanged: fully open, zero partial resolution. Broader 5-seed coverage across the remaining 3 scenarios, channel-matched comparison, DQN-policy inclusion, and any human sign-off accepting the Operational Definitions Proposal all remain open, separate questions.

---

## U06 — Seed-Repetition Report (Injected Vibration Replay) Implementation Note (IMPLEMENTATION RECORD, NOT A DECISION)
**(U06 REMAINS FULLY UNDECIDED/OPEN. This is an implementation record for the seventh U06 seed-repetition increment — it establishes 5-seed coverage for a seventh scenario shape (injected_vibration_replay), per the "U06 -- RL REWARD SHAPING SPECIFICATION PROPOSAL" section's own §4 proposed minimum. It defines no new axis, metric, threshold, or verdict, and does not resolve U06.)**

- **Date implemented:** 2026-09-06
- **Files changed:** `edge/eval/rl_baseline_eval.py` (additive only — 4 new `ScenarioConfig` constants plus one grouping tuple; no existing scenario, function signature, or behavior changed), `edge/eval/u06_seed_repetition_report_injected_vibration_replay.py` (new), `edge/tests/test_u06_seed_repetition_report_injected_vibration_replay.py` (new). No other file modified — `EpisodeRecord`, `TransitionRecord`, `active_injection_labels`, all three existing axis modules, `u06_diagnostic_report.py`, the existing clean-degradation, injected_current_spike, injected_temperature_drift, injected_pressure_stuck_at, injected_humidity_bias_fdi, and injected_gas_ramp_fdi seed-repetition modules, `edge/eval/rl_training.py`, `edge/rl/reward.py`, `edge/rl/fallback_gate.py`, `edge/rl/policy.py`, `edge/rl/environment.py`, and every hardware/driver/actuator/relay/GPIO file are untouched.

**What this increment does:**
- Adds `SCENARIO_INJECTED_VIBRATION_REPLAY_SEED_1370/1371/1372/1373` (seeds 1370–1373, none colliding with any existing evaluation-scenario seed 1337–1369 or `TRAINING_SCENARIOS` seed 2001/2002), each reusing `SCENARIO_INJECTED_VIBRATION_REPLAY`'s exact profile (length, health range, degradation rate, channel config, and its `Replay` injection unchanged) with only `seed` differing, plus a grouping tuple `INJECTED_VIBRATION_REPLAY_SEED_REPETITION_SCENARIOS` (the original plus all four variants — 5 total).
- Adds a pure function, `build_seed_repetition_report()`, in a new sibling module mirroring the six existing seed-repetition reports' exact structure — running both existing baselines over all 5 seeds and calling all three existing axis summary functions unmodified on each of the resulting 10 `EpisodeRecord`s. No average, pooled rate, min, max, range, or variability statistic across seeds is computed anywhere (confirmed by the same AST-based division/builtin-statistic checks used in all six prior increments).
- **This is now the seventh of nine scenario shapes with 5-seed coverage.** Includes an optional `main()` printer matching the existing diagnostic-entry convention.
- **Replay validation-constraint check:** unlike every prior seed-repetition scenario, `Replay` overrides its own `_validate_against()` (requiring `source_onset + duration <= len(frames)` and `source_onset + duration <= onset`). Since every seed variant changes only `seed` — never `length`, `onset`, `duration`, or `source_onset` — this constraint depends on none of the changed fields; verified directly that all five `ScenarioConfig`s construct without error and satisfy both inequalities explicitly.
- **Empirical note (diagnostic, not a claim):** for this scenario and episode length, all 5 seeds produce byte-identical `cumulative_reward`/`final_health`/axis-summary values — the same behavior already observed for the clean-degradation, injected_current_spike, injected_temperature_drift, injected_pressure_stuck_at, injected_humidity_bias_fdi, and injected_gas_ramp_fdi seed sets (now the seventh scenario shape to show the identical pattern, this time even though the injected channel itself is the one non-flat, time-varying signal in these synthetic scenarios). This is reported as a diagnostic observation about the current degradation-generator/scenario scale, for transparency only — it is NOT investigated, explained, or resolved by this increment, and must NOT be read as proof that seed variation is impossible, that executions were pooled or shared, or that the policy is accurate or safe. This disclaimer is documented directly in the module's own docstring, not only in this note.
- **Replay-on-vibration scenario note:** unlike every previously-covered injected channel (all unconfigured, flat baseline constants), `vibration` is the one channel explicitly configured in the shared evaluation profile, so its ambient value genuinely progresses over the episode — verified directly: ambient vibration rises from `0.03` at frame 0 to roughly `1.11` by frame 36. `Replay` overwrites frames `[30, 35)` with the channel's own earlier values from frames `[0, 5)`, producing a large, observable drop relative to the ambient trajectory's own progressed value at those frames; the two streams are identical outside the window. This is documented as a scenario-specific characteristic only, explicitly not a claim about how a real replay-style false-data-injection attack would behave in practice or this pipeline's real-world attack-detection capability.

**What this increment explicitly does NOT do:**
- **Does not complete §4's proposed minimum** — 2 injection-type scenarios (ConstantSpoof, AdaptiveStealthFDI) still have only their single, original seed each; broader 5-seed coverage across them remains unfinished.
- Defines no new U06 axis, comparison, definition, denominator, or metric.
- Computes no threshold, verdict, or pass/fail judgment, and makes no real-world accuracy, safety, effectiveness, validation, or production-readiness claim anywhere.
- Does not implement channel matching or DQN-policy evaluation.
- Does not modify any of the six prior seed-repetition modules — all seven exist side by side, structurally identical in shape.
- No permanent numeric snapshot from running this report is pasted into this document.

**Not done by this increment:** U06's status in the UNDECIDED list above is unchanged: fully open, zero partial resolution. Broader 5-seed coverage across the remaining 2 scenarios, channel-matched comparison, DQN-policy inclusion, and any human sign-off accepting the Operational Definitions Proposal all remain open, separate questions.

---

## U06 — Seed-Repetition Report (Injected Current Constant Spoof) Implementation Note (IMPLEMENTATION RECORD, NOT A DECISION)
**(U06 REMAINS FULLY UNDECIDED/OPEN. This is an implementation record for the eighth U06 seed-repetition increment — it establishes 5-seed coverage for an eighth scenario shape (injected_current_constant_spoof), per the "U06 -- RL REWARD SHAPING SPECIFICATION PROPOSAL" section's own §4 proposed minimum. It defines no new axis, metric, threshold, or verdict, and does not resolve U06.)**

- **Date implemented:** 2026-09-06
- **Files changed:** `edge/eval/rl_baseline_eval.py` (additive only — 4 new `ScenarioConfig` constants plus one grouping tuple; no existing scenario, function signature, or behavior changed), `edge/eval/u06_seed_repetition_report_injected_current_constant_spoof.py` (new), `edge/tests/test_u06_seed_repetition_report_injected_current_constant_spoof.py` (new). No other file modified — `EpisodeRecord`, `TransitionRecord`, `active_injection_labels`, all three existing axis modules, `u06_diagnostic_report.py`, the existing clean-degradation, injected_current_spike, injected_temperature_drift, injected_pressure_stuck_at, injected_humidity_bias_fdi, injected_gas_ramp_fdi, and injected_vibration_replay seed-repetition modules, `edge/eval/rl_training.py`, `edge/rl/reward.py`, `edge/rl/fallback_gate.py`, `edge/rl/policy.py`, `edge/rl/environment.py`, and every hardware/driver/actuator/relay/GPIO file are untouched.

**What this increment does:**
- Adds `SCENARIO_INJECTED_CURRENT_CONSTANT_SPOOF_SEED_1374/1375/1376/1377` (seeds 1374–1377, none colliding with any existing evaluation-scenario seed 1337–1373 or `TRAINING_SCENARIOS` seed 2001/2002), each reusing `SCENARIO_INJECTED_CURRENT_CONSTANT_SPOOF`'s exact profile (length, health range, degradation rate, channel config, and its `ConstantSpoof` injection unchanged) with only `seed` differing, plus a grouping tuple `INJECTED_CURRENT_CONSTANT_SPOOF_SEED_REPETITION_SCENARIOS` (the original plus all four variants — 5 total).
- Adds a pure function, `build_seed_repetition_report()`, in a new sibling module mirroring the seven existing seed-repetition reports' exact structure — running both existing baselines over all 5 seeds and calling all three existing axis summary functions unmodified on each of the resulting 10 `EpisodeRecord`s. No average, pooled rate, min, max, range, or variability statistic across seeds is computed anywhere (confirmed by the same AST-based division/builtin-statistic checks used in all seven prior increments).
- **This is now the eighth of nine scenario shapes with 5-seed coverage.** Includes an optional `main()` printer matching the existing diagnostic-entry convention.
- **Empirical note (diagnostic, not a claim):** for this scenario and episode length, all 5 seeds produce byte-identical `cumulative_reward`/`final_health`/axis-summary values — the same behavior already observed for the clean-degradation, injected_current_spike, injected_temperature_drift, injected_pressure_stuck_at, injected_humidity_bias_fdi, injected_gas_ramp_fdi, and injected_vibration_replay seed sets (now the eighth scenario shape to show the identical pattern). This is reported as a diagnostic observation about the current degradation-generator/scenario scale, for transparency only — it is NOT investigated, explained, or resolved by this increment, and must NOT be read as proof that seed variation is impossible, that executions were pooled or shared, or that the policy is accurate or safe. This disclaimer is documented directly in the module's own docstring, not only in this note.
- **ConstantSpoof-on-current scenario note:** unlike every previously-covered injection mechanism (`StuckAt`'s freeze-at-existing-value no-op, `BiasFDI`'s fixed additive offset, `RampFDI`'s growing additive offset), `ConstantSpoof` unconditionally pins the channel to a fixed, caller-chosen absolute value, independent of the underlying signal. Verified directly: ambient `current` is flat at `0.42` (unconfigured in the shared evaluation profile, same mechanism as pressure/humidity/gas), and the pinned value (`0.0`) differs from it, so — unlike the injected_pressure_stuck_at scenario's no-op — this injection is observable, with the two streams differing by exactly `0.42` throughout `[30, 35)` and identical outside it. This is documented as a scenario-specific characteristic only, explicitly not a claim about how a real constant-value spoofing attack would behave in practice or this pipeline's real-world attack-detection capability.

**What this increment explicitly does NOT do:**
- **Does not complete §4's proposed minimum** — 1 injection-type scenario (AdaptiveStealthFDI) still has only its single, original seed; broader 5-seed coverage for it remains unfinished.
- Defines no new U06 axis, comparison, definition, denominator, or metric.
- Computes no threshold, verdict, or pass/fail judgment, and makes no real-world accuracy, safety, effectiveness, validation, or production-readiness claim anywhere.
- Does not implement channel matching or DQN-policy evaluation.
- Does not modify any of the seven prior seed-repetition modules — all eight exist side by side, structurally identical in shape.
- No permanent numeric snapshot from running this report is pasted into this document.

**Not done by this increment:** U06's status in the UNDECIDED list above is unchanged: fully open, zero partial resolution. Broader 5-seed coverage across the remaining 1 scenario, channel-matched comparison, DQN-policy inclusion, and any human sign-off accepting the Operational Definitions Proposal all remain open, separate questions.

## U06 — Seed-Repetition Report (Injected Temperature Adaptive Stealth FDI) Implementation Note (IMPLEMENTATION RECORD, NOT A DECISION)
**(U06 REMAINS FULLY UNDECIDED/OPEN. This is an implementation record for the ninth and final U06 seed-repetition increment — it establishes 5-seed coverage for the ninth and last scenario shape (injected_temperature_adaptive_stealth_fdi), per the "U06 -- RL REWARD SHAPING SPECIFICATION PROPOSAL" section's own §4 proposed minimum. It defines no new axis, metric, threshold, or verdict, and does not resolve U06.)**

- **Date implemented:** 2026-09-06
- **Files changed:** `edge/eval/rl_baseline_eval.py` (additive only — 4 new `ScenarioConfig` constants plus one grouping tuple; no existing scenario, function signature, or behavior changed), `edge/eval/u06_seed_repetition_report_injected_temperature_adaptive_stealth_fdi.py` (new), `edge/tests/test_u06_seed_repetition_report_injected_temperature_adaptive_stealth_fdi.py` (new). No other file modified — `EpisodeRecord`, `TransitionRecord`, `active_injection_labels`, all three existing axis modules, `u06_diagnostic_report.py`, the existing clean-degradation, injected_current_spike, injected_temperature_drift, injected_pressure_stuck_at, injected_humidity_bias_fdi, injected_gas_ramp_fdi, injected_vibration_replay, and injected_current_constant_spoof seed-repetition modules, `edge/eval/rl_training.py`, `edge/rl/reward.py`, `edge/rl/fallback_gate.py`, `edge/rl/policy.py`, `edge/rl/environment.py`, and every hardware/driver/actuator/relay/GPIO file are untouched.

**What this increment does:**
- Adds `SCENARIO_INJECTED_TEMPERATURE_ADAPTIVE_STEALTH_FDI_SEED_1378/1379/1380/1381` (seeds 1378–1381, none colliding with any existing evaluation-scenario seed 1337–1377 or `TRAINING_SCENARIOS` seed 2001/2002), each reusing `SCENARIO_INJECTED_TEMPERATURE_ADAPTIVE_STEALTH_FDI`'s exact profile (length, health range, degradation rate, channel config, and its `AdaptiveStealthFDI` injection unchanged) with only `seed` differing, plus a grouping tuple `INJECTED_TEMPERATURE_ADAPTIVE_STEALTH_FDI_SEED_REPETITION_SCENARIOS` (the original plus all four variants — 5 total).
- Adds a pure function, `build_seed_repetition_report()`, in a new sibling module mirroring the eight existing seed-repetition reports' exact structure — running both existing baselines over all 5 seeds and calling all three existing axis summary functions unmodified on each of the resulting 10 `EpisodeRecord`s. No average, pooled rate, min, max, range, or variability statistic across seeds is computed anywhere (confirmed by the same AST-based division/builtin-statistic checks used in all eight prior increments).
- **This is now the ninth and final scenario shape with 5-seed coverage — this increment completes the proposed 5-seed minimum across all 9 of 9 synthetic scenario shapes in the taxonomy.** Includes an optional `main()` printer matching the existing diagnostic-entry convention.
- **Empirical note (diagnostic, not a claim):** for this scenario and episode length, all 5 seeds produce byte-identical `cumulative_reward`/`final_health`/axis-summary values — the same behavior already observed for the clean-degradation, injected_current_spike, injected_temperature_drift, injected_pressure_stuck_at, injected_humidity_bias_fdi, injected_gas_ramp_fdi, injected_vibration_replay, and injected_current_constant_spoof seed sets (now the ninth and final scenario shape to show the identical pattern). This is reported as a diagnostic observation about the current degradation-generator/scenario scale, for transparency only — it is NOT investigated, explained, or resolved by this increment, and must NOT be read as proof that seed variation is impossible, that executions were pooled or shared, or that the policy is accurate or safe. This disclaimer is documented directly in the module's own docstring, not only in this note.
- **AdaptiveStealthFDI-on-temperature scenario note:** unlike every previously-covered injection mechanism (`StuckAt`'s freeze-at-existing-value no-op, `BiasFDI`'s fixed additive offset, `RampFDI`'s unbounded-growing additive offset, `ConstantSpoof`'s absolute-value pin), `AdaptiveStealthFDI` adds a per-sample offset that grows at a fixed rate only until it hits a fixed cap, then holds flat at that cap. Verified directly, frame-by-frame: ambient `temperature` is flat at `26.0` (unconfigured in the shared evaluation profile, same mechanism as pressure/humidity/gas/current), and during the active window `[25, 35)` the offset is `+0.5, +1.0, +1.5, +2.0` for the first four active steps, then holds at the capped `+2.0` for the remaining six active steps — the two streams are identical outside `[25, 35)`. This is documented as a scenario-specific characteristic only, explicitly not a claim about how a real adaptive or stealth false-data-injection attack would behave in practice or this pipeline's real-world attack-detection capability.

**What this increment explicitly does NOT do:**
- **This completes the proposed five-seed coverage across all nine synthetic scenario shapes** — the remaining 0 injection-type scenarios are unfinished. This completion is strictly scoped to §4's proposed 5-distinct-seeds-per-scenario minimum: it does **not** resolve U06, does **not** establish policy accuracy, safety, effectiveness, real-world validation, or production readiness, and does **not** constitute human sign-off accepting the Operational Definitions Proposal itself.
- Defines no new U06 axis, comparison, definition, denominator, or metric.
- Computes no threshold, verdict, or pass/fail judgment, and makes no real-world accuracy, safety, effectiveness, validation, or production-readiness claim anywhere.
- Does not implement channel matching or DQN-policy evaluation.
- Does not modify any of the eight prior seed-repetition modules — all nine exist side by side, structurally identical in shape.
- No permanent numeric snapshot from running this report is pasted into this document.
...
## U06 — Injected-Channel Plumbing Implementation Note (IMPLEMENTATION RECORD, NOT A DECISION)
**(U06 REMAINS FULLY UNDECIDED/OPEN. This is an implementation record for the smallest approved increment scoped in the channel-matched-comparison feasibility review above — it plumbs the second (and final) already-available data field a future, separately approved channel-comparison increment would need, alongside the already-committed `tracked_channels` field. It defines no comparison, axis, metric, threshold, or verdict, and does not resolve U06.)**

- **Date implemented:** 2026-09-06
- **Files changed:** `edge/eval/rl_baseline_eval.py`, `edge/tests/test_rl_baseline_eval.py`. No reward, policy, fallback-gate, environment, training, hardware, DQN, or existing axis-summary file changed, and `edge/injection/injections.py` was not modified — the channel data this increment reads (`Label.channel`) was already defined there and already retrieved into `active_labels` at the existing `_run_episode` construction site; this increment only reads one field of an object already in scope.

**What this increment does:**
- `edge/eval/rl_baseline_eval.py`'s `TransitionRecord` gains one new, defaulted field, `active_injection_channels: tuple[str, ...] = ()` — raw channel-name strings for each currently active injection (e.g. `("current",)`), populated in `_run_episode` via `tuple(label.channel for label in active_labels)` — the exact same `active_labels` tuple `active_injection_labels` already reads, just extracting `.channel` instead of `.injection_type.value`. No new lookup, environment access, or `ScenarioConfig`/scenario-name inference was introduced.
- Verified directly: for `SCENARIO_INJECTED_CURRENT_SPIKE`, `active_injection_channels` reports exactly `("current",)` at every step where `active_injection_labels` reports `("spike",)` — the two fields stay positionally aligned, in the same order, since both are built from the same underlying tuple.
- Verified directly via a test-only, non-committed multi-injection `ScenarioConfig` (two simultaneous injections, `Spike(channel="current", ...)` and `Drift(channel="temperature", ...)`, mirroring `edge/tests/test_rl_environment.py`'s own existing multi-injection test precedent): both channels are retained, in deterministic order, with no overwriting — `active_injection_channels` and `active_injection_labels` both have length 2 at the overlapping steps.

**What this increment explicitly does NOT do:**
- Does not implement channel-matched comparison — `active_injection_channels` is not compared against `tracked_channels` or anything else anywhere in this module; no such comparison, agreement rate, threshold, or verdict is computed, stored, or claimed (confirmed by a dedicated source-scan test).
- Does not modify `edge/injection/injections.py`, `edge/rl/fallback_gate.py`, `edge/rl/environment.py`, `edge/rl/policy.py`, `edge/rl/reward.py`, `edge/eval/rl_training.py`, or any of the three existing axis-summary modules — all read `TransitionRecord` unmodified and are unaffected by this additive field.
- Does not change any existing scenario's behavior, any seed-repetition report, or the aggregate diagnostic report — all continue to produce byte-identical results to before this increment (the new field is additive and ignored by every existing consumer).
- No false-isolation rate, missed-critical-fault rate, threshold, verdict, pooling, averaging, or confidence interval is computed, stored, or claimed anywhere.
- No real-world fault-detection, attack-detection, accuracy, safety, effectiveness, validation, or production-readiness claim is made.
...
## U06 — Sample_seq Plumbing and Invariant Proof Implementation Note (IMPLEMENTATION RECORD, NOT A DECISION)
**(U06 REMAINS FULLY UNDECIDED/OPEN. This is an implementation record for the smaller of two options scoped in the sample_seq-deduplication design proposal above — the U06 decision owner explicitly approved "plumbing + invariant proof only," deferring any runtime deduplication logic as a separate, larger, not-yet-approved decision. It defines no deduplication pass and does not resolve U06.)**

- **Date implemented:** 2026-09-06
- **Files changed:** `edge/eval/rl_baseline_eval.py`, `edge/tests/test_rl_baseline_eval.py`. No other file modified — `edge/eval/u06_tracker_agreement.py` and `edge/eval/u06_channel_agreement.py` (the only two modules a deduplication pass could ever apply to) are explicitly untouched, as are axis (i), axis (iii), the aggregate diagnostic report, every seed-repetition report, `edge/injection/injections.py`, `edge/rl/fallback_gate.py`, `edge/rl/environment.py`, `edge/rl/policy.py`, `edge/rl/reward.py`, and `edge/eval/rl_training.py`.

**What this increment does:**
- `TransitionRecord` gains one new, defaulted field, `sample_seq: int | None = None`, populated in `_run_episode` directly from the same local `sample_seq` value already computed at the existing construction site (previously used only transiently to look up injection labels, then discarded). No new lookup, environment access, or additional `result.info` read was introduced.
- Adds a diagnostic proof, not a deduplication pass: across every committed evaluation scenario and both baselines, `sample_seq` values among `transition_consumed=True` transitions are verified to be unique and strictly increasing within each episode — the invariant `edge/rl/environment.py`'s own cursor-advancement logic (`_feed_next_frame` always advances by exactly one frame, or raises rather than repeating one) and immediate-termination-on-`safe_stop` logic (the episode ends the instant `safe_stop` is approved, so at most one `transition_consumed=False` transition can ever exist per episode) together guarantee.
- **Verified directly: no currently-committed scenario, under either baseline, ever produces a `transition_consumed=False` transition at all** — every episode either exhausts its trajectory or never triggers `safe_stop`. Because of this, three additional synthetic (directly-constructed, non-committed) `TransitionRecord` fixtures were added to prove the invariant check itself is not vacuous: one with a deliberately duplicated `sample_seq` (confirmed to fail the check), one with `sample_seq=None` on both transitions (confirmed to be excluded from the check entirely, never compared as if they were equal or unequal), and one where a `transition_consumed=False` transition deliberately echoes the preceding real step's `sample_seq` (confirmed to be excluded from the check, exactly matching the documented safe_stop echo mechanism).
- **Practical conclusion: this increment changes no currently-reported number anywhere.** Every existing axis (i)/(ii)/(iii), diagnostic report, seed-repetition report, and channel-agreement output is unaffected — this is a proof that the guarantee those modules' own docstrings already assert in prose actually holds, not a correction to anything.

**What this increment explicitly does NOT do:**
- Implements no deduplication pass, filter, or drop-duplicate operation anywhere — confirmed by a dedicated source-scan test asserting no such logic exists in `edge/eval/rl_baseline_eval.py`.
- Does not modify `edge/eval/u06_tracker_agreement.py` or `edge/eval/u06_channel_agreement.py` — both remain byte-identical to before this increment, and their existing `transition_consumed=True` observation-selection filters are untouched.
- Does not implement axis (i) or axis (iii) changes, DQN-policy evaluation, confidence-interval/uncertainty reporting, or any threshold, verdict, pass/fail gate, or reward-weight change.
- No real-world fault-detection, attack-detection, accuracy, safety, effectiveness, validation, or production-readiness claim is made.
