...
- [x] **C1** sensor driver abstraction — `SensorDriver`/`Reading` interface + `Sensor` (calibration, range-clamp, health, ts) + fakes (511537c). Real driver bodies now exist and are each individually hardware-validated for five of six channels: `edge/drivers/{adxl335,bmp280,ds18b20,ina219,dht22}.py` (vibration/pressure/temperature/current/humidity — BMP280 replaces the originally-documented BMP180 per D027, added `3e34bd1`/2026-09-03). Gas (MQ-135) still has no driver. **Current bench state (`8fa7363`, 2026-09-04): only DS18B20 (temperature) is physically connected** — `edge/main.py` wires it as real and the other five channels as `fake_drivers()`; ADXL335 and BMP280 were each briefly connected alongside it earlier the same week (`d419d6b`, `adc2eb6`) before the bench was reconfigured down to DS18B20 alone. See `CURRENT_STATE.md`'s P1 physical-hardware bullet and `IMPLEMENTATION_LOG.md`'s 2026-09-03/09-04 entries for full detail.
