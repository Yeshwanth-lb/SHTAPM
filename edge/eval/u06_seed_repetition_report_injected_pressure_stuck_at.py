"""U06 seed-repetition report -- injected_pressure_stuck_at scenario shape
...
u06_ground_truth_rate_summary``) across the five injected_pressure_stuck_at
seed variants in ``edge.eval.rl_baseline_eval.
INJECTED_PRESSURE_STUCK_AT_SEED_REPETITION_SCENARIOS`` for both existing
deterministic baselines, reporting every (seed, baseline) result
independently -- this is the FOURTH scenario shape covered by the U06
seed-repetition pattern (after ``edge.eval.u06_seed_repetition_report``'s
own clean-degradation coverage, ``edge.eval.
u06_seed_repetition_report_injected_current_spike``'s own coverage, and
...
share one shape (a single, unchanged pressure-channel ``StuckAt``
...
modify any of the three existing seed-repetition modules (clean
degradation, injected_current_spike, injected_temperature_drift) -- all
four modules exist side by side, structurally identical in shape, each
scoped to its own scenario.
...
STILL PARTIAL COVERAGE OF §4: with this module, 4 of the 9
evaluation-scenario shapes (clean degradation, injected_current_spike,
injected_temperature_drift, injected_pressure_stuck_at) now have 5-seed
coverage. §4's proposed minimum applies to EVERY scenario in the
taxonomy -- the remaining 5 injection-type scenarios (BiasFDI, RampFDI,
Replay, ConstantSpoof, AdaptiveStealthFDI) still have only their single,
original seed each. Broader 5-seed coverage across those 5 remains
unfinished and is NOT implemented here.

STUCKAT-ON-PRESSURE CHARACTERISTIC (a property of the already-committed
synthetic scenario, restated here for accuracy, NOT a real-world
conclusion): ``StuckAt(channel="pressure", onset=30, duration=5)`` uses
its default ``held_value=None``, which freezes the channel at whatever raw
value it had at frame index ``onset`` (see ``edge/injection/
injections.py``'s own ``StuckAt._value()``). Because ``pressure`` is NOT
one of the channels configured in ``edge.eval.rl_baseline_eval.
_EVALUATION_DEGRADATION_PROFILE`` (only ``vibration`` is), the degradation
generator returns a flat, unconfigured baseline constant for ``pressure``
on every frame regardless of index or seed (see ``edge/models/
degradation_generator.py``'s own ``_channel_sample()`` "unconfigured"
branch). Verified directly for this exact scenario: the injected and
uninjected ``pressure`` streams are byte-for-byte identical at every
frame. Consequently, freezing ``pressure`` at its already-constant ambient
value produces no observable change to the wire values at all -- this
injection is, for this specific synthetic scenario's configuration,
functionally a no-op against its own baseline, unlike ``Drift``/``Spike``
(both of which add an offset that IS observably different from baseline).
This is stated as a characteristic of this synthetic scenario's own
configuration, not as any claim about how a real stuck-at sensor fault
would behave, nor as evidence for or against this pipeline's real-world
fault-detection capability.

EMPIRICAL NOTE (diagnostic observation, not investigated or resolved by
this increment): all 5 ``injected_pressure_stuck_at`` seeds currently
...
spike, and injected_temperature_drift seed sets -- this is now the FOURTH
scenario shape to show the identical pattern. As with those prior
increments, this is reported here as a diagnostic observation about the
current degradation-generator/scenario scale, for transparency only -- it
is NOT investigated, explained, or resolved by this increment, and it
must NOT be interpreted as proof that seed variation is impossible, that
these executions were pooled or shared, or that the policy is accurate or
safe. (Separately, the STUCKAT-ON-PRESSURE CHARACTERISTIC above traces one
concrete, verified reason a scenario built from this shared profile could
show no seed sensitivity for its own injected channel -- that observation
is scenario-specific and does not itself explain the SAME pattern already
seen on ``vibration``/``current``/``temperature`` in the three prior
scenarios, which is still not investigated by this increment.)
...
from edge.eval.rl_baseline_eval import (
    INJECTED_PRESSURE_STUCK_AT_SEED_REPETITION_SCENARIOS,
    EpisodeRecord,
    ScenarioConfig,
...
class SeedRepetitionReport:
    ...
    injected_pressure_stuck_at scenario shape -- see module docstring.
...
def build_seed_repetition_report() -> SeedRepetitionReport:
    ...
    baseline) pair across the five injected_pressure_stuck_at seed
    ...
    exactly ``len(INJECTED_PRESSURE_STUCK_AT_SEED_REPETITION_SCENARIOS) *
...
def main() -> None:
    ...
    ``python -m edge.eval.u06_seed_repetition_report_injected_pressure_stuck_at``.
    Prints a concise, per-seed-separated summary of all 10 (seed, baseline)
    results for the injected_pressure_stuck_at scenario shape. NOT a
    validation claim, NOT a pass/fail judgment, and NOT a cross-seed
    summary -- see module docstring."""
    print(
        "=== U06 seed-repetition report: injected_pressure_stuck_at, 5 seeds "
        "(simulation-only) ==="
    )
    ...
    print(
        ...
        "only the injected_pressure_stuck_at scenario shape -- broader 5-seed "
        "coverage across the remaining 5 injection-type scenarios remains "
        "unfinished."
    )
