"""U06 seed-repetition report -- injected_current_spike scenario shape --
pure, additive, opt-in. Runs the four existing, independently-committed
...
u06_channel_agreement``) across the five injected_current_spike
seed variants in ``edge.eval.rl_baseline_eval.
INJECTED_CURRENT_SPIKE_SEED_REPETITION_SCENARIOS`` for both existing
...
share one shape (a single, unchanged current-channel ``Spike`` injection
on the same clean-degradation-shaped trajectory) and differ only in seed.
It never constructs an environment, gate, reward, or policy object
...
module) -- both modules exist side by side, structurally identical in
shape, each scoped to its own scenario.
...
Broader 5-seed coverage across those 7 remains unfinished and is NOT
implemented here.

EMPIRICAL NOTE (diagnostic observation, not investigated or resolved by
this increment): all 5 ``injected_current_spike`` seeds currently produce
...
already observed for the clean-degradation seed set
(``edge.eval.u06_seed_repetition_report``). This is reported here as a
...
seed variation is impossible, that these executions were pooled or
shared, or that the policy is accurate or safe.
...
class SeedRepetitionReport:
    ...
    injected_current_spike scenario shape -- see module docstring.
...
def build_seed_repetition_report() -> SeedRepetitionReport:
    ...
    baseline) pair across the five injected_current_spike seed variants,
    ...
    ``len(INJECTED_CURRENT_SPIKE_SEED_REPETITION_SCENARIOS) * len(
...
def main() -> None:
    ...
    ``python -m edge.eval.u06_seed_repetition_report_injected_current_spike``.
    Prints a concise, per-seed-separated summary of all 10 (seed, baseline)
    results for the injected_current_spike scenario shape. NOT a
    ...
    print(
        ...
        "only the injected_current_spike scenario shape -- broader 5-seed "
        "coverage across the remaining 7 injection-type scenarios remains "
        "unfinished."
    )
