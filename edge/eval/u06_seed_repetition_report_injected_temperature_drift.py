"""U06 seed-repetition report -- injected_temperature_drift scenario shape
...
u06_ground_truth_rate_summary``) across the five injected_temperature_drift
seed variants in ``edge.eval.rl_baseline_eval.
INJECTED_TEMPERATURE_DRIFT_SEED_REPETITION_SCENARIOS`` for both existing
deterministic baselines, reporting every (seed, baseline) result
independently -- this is the THIRD scenario shape covered by the U06
seed-repetition pattern (after ``edge.eval.u06_seed_repetition_report``'s
own clean-degradation coverage and ``edge.eval.
u06_seed_repetition_report_injected_current_spike``'s own coverage), per
the "U06 -- Operational Definitions Proposal"'s own section D (proposed
minimum: at least 5 distinct seeds per scenario).
...
share one shape (a single, unchanged temperature-channel ``Drift``
...
modify ``edge.eval.u06_seed_repetition_report`` (the clean-degradation
module) or ``edge.eval.u06_seed_repetition_report_injected_current_spike``
(the injected_current_spike module) -- all three modules exist side by
...
STILL PARTIAL COVERAGE OF SECTION D: with this module, 3 of the 9
evaluation-scenario shapes (clean degradation, injected_current_spike,
injected_temperature_drift) now have 5-seed coverage. Section D's proposed
minimum applies to EVERY scenario in the taxonomy -- the remaining 6
injection-type scenarios (StuckAt, BiasFDI, RampFDI, Replay, ConstantSpoof,
AdaptiveStealthFDI) still have only their single, original seed each.
Broader 5-seed coverage across those 6 remains unfinished and is NOT
implemented here.

EMPIRICAL NOTE (diagnostic observation, not investigated or resolved by
this increment): all 5 ``injected_temperature_drift`` seeds currently
...
behavior was already observed for both the clean-degradation seed set
(``edge.eval.u06_seed_repetition_report``) and the injected_current_spike
seed set (``edge.eval.u06_seed_repetition_report_injected_current_spike``)
-- this is now the THIRD scenario shape to show the identical pattern.
This is reported here as a diagnostic observation about the current
degradation-generator/scenario scale, for transparency only -- it is NOT
investigated, explained, or resolved by this increment, and it must NOT
be interpreted as proof that seed variation is impossible, that these
executions were pooled or shared, or that the policy is accurate or safe.
...
class SeedRepetitionReport:
    ...
    injected_temperature_drift scenario shape -- see module docstring.
...
def build_seed_repetition_report() -> SeedRepetitionReport:
    ...
    baseline) pair across the five injected_temperature_drift seed
    ...
    exactly ``len(INJECTED_TEMPERATURE_DRIFT_SEED_REPETITION_SCENARIOS) *
...
def main() -> None:
    ...
    ``python -m edge.eval.u06_seed_repetition_report_injected_temperature_drift``.
    Prints a concise, per-seed-separated summary of all 10 (seed, baseline)
    results for the injected_temperature_drift scenario shape. NOT a
    validation claim, NOT a pass/fail judgment, and NOT a cross-seed
    summary -- see module docstring."""
    print(
        "=== U06 seed-repetition report: injected_temperature_drift, 5 seeds "
        "(simulation-only) ==="
    )
    ...
    print(
        ...
        "only the injected_temperature_drift scenario shape -- broader 5-seed "
        "coverage across the remaining 6 injection-type scenarios remains "
        "unfinished."
    )
